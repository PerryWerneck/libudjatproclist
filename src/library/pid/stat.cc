...
/**
 *
 * @page pidstat /proc/pid/stat explained
 *
 ...
 *								in bits 15 to 8.)
 *
 *	- (8) tpgid			%d		The ID of the foreground process group of the
 *								controlling terminal of the process.
 *
 *	- (9) flags			%u		The kernel flags word of the process.  For bit
 ...
 *								sysconf(_SC_CLK_TCK)).
 *
 *	- (16) cutime		%ld		Amount of time that this process's waited-for
 ...
 *								below).
 *
 *	- (17) cstime  		%ld		Amount of time that this process's waited-for
 *								children have been scheduled in kernel mode,
 *								measured in clock ticks (divide by
 *								sysconf(_SC_CLK_TCK)).
 *
 *	- (18) priority		%ld		(Explanation for Linux 2.6) For processes running a
 ...
 *
 *	- (21) itrealvalue 	 %ld	The time in jiffies before the next SIGALRM is sent
 *								to the process due to an interval timer.  Since
 *								kernel 2.6.17, this field is no longer maintained,
 *								and is hard coded as 0.
 *
 *	- (22) starttime  	%llu	The time the process started after system boot.  In
 ...
 *
 *	- (25) rsslim  		%lu		Current soft limit in bytes on the rss of the
 ...
 *
 *	- (31) signal		%lu		The bitmap of pending signals, displayed as a
 *								decimal number.  Obsolete, because it does not
 *								provide information on real-time signals; use
 *								/proc/[pid]/status instead.
 *
 *	- (32) blocked		%lu		The bitmap of blocked signals, displayed as a
 *								decimal number.  Obsolete, because it does not
 *								provide information on real-time signals; use
 *								/proc/[pid]/status instead.
 *
 *	- (33) sigignore	%lu		The bitmap of ignored signals, displayed as a
 *								decimal number.  Obsolete, because it does not
 *								provide information on real-time signals; use
 *								/proc/[pid]/status instead.
 *
 *	- (34) sigcatch 	%lu		The bitmap of caught signals, displayed as a decimal
 *								number.  Obsolete, because it does not provide
 *								information on real-time signals; use
 *								/proc/[pid]/status instead.
 *
 *	- (35) wchan		%lu		This is the "channel" in which the process is
 ...
 *
 *	- (40) rt_priority	%u		(since Linux 2.5.19) Real-time scheduling priority, a number in the range
 *								1 to 99 for processes scheduled under a real-time
 *								policy, or 0, for non-real-time processes (see
 *								sched_setscheduler(2)).
 *
 *	- (41) policy		%u		(since Linux 2.5.19) Scheduling policy (see sched_setscheduler(2)).
 *								Decode using the SCHED_* constants in linux/sched.h.
 *
 *								The format for this field was %lu before Linux 2.6.22.
 *
 *	- (42) blkio_ticks	%llu	(since Linux 2.6.18) Aggregated block I/O delays, measured in clock ticks (centiseconds).
 *
 *	- (43) guest_time	%lu		(since Linux 2.6.24) Guest time of the process (time spent running a
 *								virtual CPU for a guest operating system), measured
 *								in clock ticks (divide by sysconf(_SC_CLK_TCK)).
 *
 *	- (44) cguest_time	%ld		(since Linux 2.6.24)	Guest time of the process's children, measured in
 *								clock ticks (divide by sysconf(_SC_CLK_TCK)).
 *
 *	- (45) start_data	%lu		(since Linux 3.3) Address above which program initialized and
 *								uninitialized (BSS) data are placed.
 *
 *	- (46) end_data		%lu		(since Linux 3.3) Address below which program initialized and
 *								uninitialized (BSS) data are placed.
 *
 *	- (47) start_brk	%lu		(since Linux 3.3) Address above which program heap can be expanded
 *								with brk(2).
 *
 *	- (48) arg_start	%lu		(since Linux 3.5) Address above which program command-line arguments
 *								(argv) are placed.
 *
 *	- (49) arg_end		%lu		(since Linux 3.5) Address below program command-line arguments (argv)
 *								are placed.
 *
 ...
 *
 *	- (52) exit_code	%d		(since Linux 3.5) The thread's exit status in the form reported by waitpid(2).
 *
 */
 ...
 namespace Udjat {
	...
	Process::Identifier::Stat::Stat(pid_t pid) {
		...
		// Read just after the process name.
		char *ptr = strrchr(buffer,')');
		if(!ptr) {
			throw std::system_error(errno, std::system_category(), "Format error on /proc/pid/stat");
			return;
		}
		...
		if(sz != 50) {
			throw runtime_error("Error parsing /proc/pid/stat");
		}
